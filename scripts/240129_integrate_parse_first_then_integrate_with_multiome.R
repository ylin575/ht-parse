...
saveRDS(hthy2wi, file = "data/rds_objects/integrated_only_parse_samples_post-qc_240129.rds")
hthy2wi <- readRDS(file = "data/rds_objects/integrated_only_parse_samples_post-qc_240129.rds")
...
DimPlot(all_samples_ni, reduction = "umap", group.by = "sample")
...
# determine if there are features not shared between parse and multiome dataset

# extract features list from parse
temp1 <- as.data.frame(hthy2wi@assays$RNA@features)
parse_features <- rownames(temp1)
# extract features list from multiome
temp2 <- as.data.frame(reference@assays$RNA@counts@Dimnames[1])
multiome_features <- rownames(temp2)
...
hthy2wi@assays$RNA@features[rownames(hthy2wi@assays$RNA@features) == "MYH16"]
reference@assays$RNA@counts@Dimnames[1][reference@assays$RNA@counts@Dimnames[1] == "MYH16"]
# check class of each object from each dataset
class(hthy2wi@assays$RNA@features@dimnames[1])
class(reference@assays$RNA@counts@Dimnames[1])
rownames(temp1)
"MYH16" %in% hthy2wi@assays$RNA@features@dimnames
...
# code revision for extraction of feature names from parse and multiome

# extract features list from parse as a character vector
parse_features2 <- as.data.frame(hthy2wi@assays$RNA@features@dimnames[1])[,1]
# extract features list from multiome as a character vector
multiome_features2 <- as.data.frame(reference@assays$RNA@counts@Dimnames[1])[,1]
# might be better way: temp2 <- as.data.frame(reference@assays$RNA@counts@Dimnames[1])

# extract features unique to parse dataset
parse_unique2 <- parse_features[!(parse_features %in% multiome_features)]
multiome_unique2 <- multiome_features[!(multiome_features %in% parse_features)]
# check to see if the unique sets are truly unique
sum(parse_unique2 %in% multiome_unique2)
...
# 240201 sarah's code to look at intersection of parse and multiome datasets
length(intersect(rownames(hthy), rownames(reference)))
#[1] 21293
length(setdiff(rownames(hthy), rownames(reference)))
#[1] 18362
length(setdiff(rownames(reference), rownames(hthy)))
#[1] 15308
21293+18362+15308
#[1] 54963
...
saveRDS(hthy2wi, file = "data/rds_objects/integrated_joined_sampleIDs_240123.rds")
hthy2wi <- readRDS(file = "data/rds_objects/integrated_joined_sampleIDs_240123.rds")

# plot graphs after combining sample IDs
...
FeaturePlot(hthy2wi, reduction = "umap", features = "CD200", split.by = "orig.ident", order = T)
...
DimPlot(tec.ref, reduction = "umap", group.by = "samples", label = TRUE)
DimPlot(tec.ref, reduction = "umap", split.by = "samples")

FeaturePlot(tec.query, reduction = "umap", features = "AIRE", split.by = "samples")
FeaturePlot(tec.ref, reduction = "umap", features = "AIRE", split.by = "samples")
...
tec.query <- MapQuery(anchorset = tec.anchors, 
                      reference = tec.ref, 
                      query = tec.query,
                      refdata = list(celltype = "celltypes"), 
                      reference.reduction = "pca", reduction.model = "umap")

# plot predicted cell types
DimPlot(tec.query, reduction = "umap", group.by = "predicted.celltype", 
        split.by = "samples", label = TRUE) + 
        NoLegend()
DimPlot(tec.query, reduction = "umap")
DimPlot(tec.query, reduction = "umap", group.by = "predicted.celltype")
DimPlot(tec.query, reduction = "umap", group.by = "predicted.celltype", 
...
DimPlot(tec.query[,tec.query@meta.data$samples == "Ht14"], reduction = "umap", 
        group.by = "predicted.celltype") +
        ggtitle("HT14")
DimPlot(tec.ref, reduction = "umap", group.by = "celltypes")

# plot UMAP projection of query data onto reference data umap
p1 <- DimPlot(tec.ref, reduction = "umap", group.by = "celltypes", 
...
p3 <- DimPlot(tec.ref, reduction = "umap", group.by = "celltypes", 
              label = TRUE, label.size = 3,repel = TRUE) + 
      NoLegend() + 
      ggtitle("Reference annotations")
...
tec.query.cellcounts <- table(tec.query$predicted.celltype, 
                              by=tec.query@meta.data$samples)
tec.ref.cellcounts <- table(tec.ref$celltypes, by=tec.ref@meta.data$samples)

write.csv(tec.query.cellcounts, file = "tec_query_tab.csv")
write.csv(tec.ref.cellcounts, file = "tec_ref_tab.csv")




grep("MT", rownames(hthy2wi), value=T)
...
# normalize the data
hthy2wi <- NormalizeData(hthy2wi, normalization.method = "LogNormalize", scale.factor
                      = 10000)
...
hthy <- FindNeighbors(hthy, dims = 1:30)
hthy <- FindClusters(hthy, resolution = 0.5, 
                      cluster.name = "parse_unintegrated_clusters")

hthy <- RunUMAP(hthy, dims = 1:30, 
                 reduction.name = "parse_umap_unintegrated")
DimPlot(hthy, reduction = "parse_umap_unintegrated", group.by = 
          c( "sample"))
...
# merge > integrate > plot
hthy3 <- merge(x = ht9_sub, y = list(ht11_sub, ht12_sub, ht14_sub))
hthy3

# run standard analysis workflow WITHOUT integration
hthy3 <- NormalizeData(hthy3)
hthy3 <- FindVariableFeatures(hthy3)
hthy3 <- ScaleData(hthy3)
hthy3 <- RunPCA(hthy3, features = VariableFeatures(object = hthy3))

hthy3 <- FindNeighbors(hthy3, dims = 1:30)
hthy3 <- FindClusters(hthy3, resolution = 0.5, 
                      cluster.name = "unintegrated_clusters")

hthy3 <- RunUMAP(hthy3, dims = 1:30, 
                 reduction.name = "umap.unintegrated")
DimPlot(hthy3, reduction = "umap.unintegrated", group.by = "sample")


# perform integration on hthy2

hthy3wi <- IntegrateLayers(object = hthy3, method = CCAIntegration, 
                           orig.reduction = "pca", 
                           new.reduction = "integrated.cca",
...
hthy3wi[["RNA"]] <- JoinLayers(hthy3wi[["RNA"]])

# save object post-integration and joining layer
# saveRDS(test1wi, file = "data/rds_objects/seurat_obj_integrated_240102.rds")
# read object post-integration and joining layer
# test1wi <- readRDS(file = "data/rds_objects/seurat_obj_integrated_240102.rds")


hthy3wi <- FindNeighbors(hthy3wi, reduction = "integrated.cca", dims = 1:30)
hthy3wi <- FindClusters(hthy3wi, resolution = 0.5)
hthy3wi <- RunUMAP(hthy3wi, dims = 1:30, reduction = "integrated.cca")

DimPlot(hthy3wi, reduction = "umap", group.by = "sample")
DimPlot(hthy3wi, reduction = "umap", split.by = "sample")

# save new seurat object into a rds object
saveRDS(hthy3wi, file = "data/rds_objects/parse_samples_integrated_240122.rds")
...
# test merge
test1 <- merge(x = hthy3wi, y = reference) # for analysis WITHOUT integration
...
test1 <- RunUMAP(test1, dims = 1:30, reduction = "pca", 
                 reduction.name = "umap.unintegrated")
DimPlot(test1, reduction = "umap.unintegrated", group.by ="sample")
DimPlot(test1, reduction = "umap.unintegrated", group.by ="batch")
...
# Visualization before combining sample IDs
...
DimPlot(test1wi, reduction = "umap", group.by = "samples", split.by = "samples")
...
# combine sample IDs
test1wi@meta.data <- test1wi@meta.data |>
  mutate(samples = ifelse(is.na(sample), batch, sample))

# Visualization after combining sample IDs
DimPlot(test1wi, reduction = "umap", split.by = "samples", group.by = "celltypes")
DimPlot(test1wi, reduction = "umap", split.by = "samples")
DimPlot(test1wi, reduction = "umap", split.by = "samples")


# plot gene expression
FeaturePlot(test1wi, features = "AIRE", split.by = "samples")
FeaturePlot(test1wi, features = "PRSS16", split.by = "samples")
FeaturePlot(test1wi, features = "PSMB11", split.by = "samples")
FeaturePlot(test1wi, features = "LY75", split.by = "samples")

# save new seurat object into a rds object
saveRDS(test1wi, file = "data/rds_objects/hthy_ref_integrated_240103.rds")
test1wi <- readRDS(file = "data/rds_objects/hthy_ref_integrated_240103.rds")



# test label transfer
tec.ref <- subset(test1wi, orig.ident == "SeuratProject")
tec.query <- subset(test1wi, orig.ident == "hthy")
...
# merge > integrate > plot
hthy3 <- merge(x = ht9_sub, y = list(ht11_sub, ht12_sub, ht14_sub, reference))
hthy3
...
hthy3wi2 <- IntegrateLayers(object = hthy3, method = CCAIntegration, 
                           orig.reduction = "pca", 
                           new.reduction = "integrated.cca",
                           verbose = FALSE) # 240102 took 15m55s
                                            # 240122 took 1hr
...
# re-join layers after integration
hthy3wi2[["RNA"]] <- JoinLayers(hthy3wi2[["RNA"]])

# save object post-integration and joining layer
saveRDS(hthy3wi2, file = "data/rds_objects/seurat_obj_integrated_240123.rds")
# read object post-integration and joining layer
# test1wi <- readRDS(file = "data/rds_objects/seurat_obj_integrated_240102.rds")


hthy3wi2 <- FindNeighbors(hthy3wi2, reduction = "integrated.cca", dims = 1:30)
hthy3wi2 <- FindClusters(hthy3wi2, resolution = 0.5)
hthy3wi2 <- RunUMAP(hthy3wi2, dims = 1:30, reduction = "integrated.cca")

DimPlot(hthy3wi2, reduction = "umap", group.by = "sample")
DimPlot(hthy3wi2, reduction = "umap", split.by = "sample")
DimPlot(hthy3wi2, reduction = "umap", split.by = "batch")
